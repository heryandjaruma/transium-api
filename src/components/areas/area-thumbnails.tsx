...
import type { AreaMedia } from "@/lib/area"
...
type Props = {
    ...
    onChange: (thumbnails: AreaMedia[]) => void
}

/** Upload/remove thumbnails for an area, and edit each one's alt text and copyright. Uploads go straight to R2 via /api/area/media. */
export function AreaThumbnails({ areaId, thumbnails, onChange }: Props) {
    ...
    function fieldsFor(media: AreaMedia) {
        return edits[media.id] ?? { alt: media.alt ?? "", copyright: media.copyright ?? "" }
    }

    function isDirty(media: AreaMedia) {
        const f = fieldsFor(media)
        return f.alt !== (media.alt ?? "") || f.copyright !== (media.copyright ?? "")
    }

    function updateField(mediaId: string, media: AreaMedia, patch: Partial<{ alt: string; copyright: string }>) {
        setEdits((prev) => ({
            ...prev,
    ...
    async function handleFileSelected(e: React.ChangeEvent<HTMLInputElement>) {
        ...
        try {
            ...
            const res = await fetch("/api/area/media", { method: "POST", body: formData })
            const data = await res.json<{ media?: AreaMedia; error?: string }>().catch(() => null)
    ...
    async function handleDelete(mediaId: string) {
        ...
        setError(null)
        try {
            const res = await fetch(`/api/area/media?areaId=${areaId}&mediaId=${mediaId}`, { method: "DELETE" })
    ...
    async function handleSave(media: AreaMedia) {
        const f = fieldsFor(media)
        setSavingId(media.id)
        setError(null)
        try {
            const res = await fetch("/api/area/media", {
                method: "PATCH",

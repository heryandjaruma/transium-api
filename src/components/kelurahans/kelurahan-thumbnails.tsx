...
import type { KelurahanMedia } from "@/lib/kelurahan"
...
type Props = {
    ...
    onChange: (thumbnails: KelurahanMedia[]) => void
}

/** Upload/remove thumbnails for a kelurahan, and edit each one's alt text and copyright. Uploads go straight to R2 via /api/kelurahan/media. */
export function KelurahanThumbnails({ kelurahanId, thumbnails, onChange }: Props) {
    ...
    function fieldsFor(media: KelurahanMedia) {
        return edits[media.id] ?? { alt: media.alt ?? "", copyright: media.copyright ?? "" }
    }

    function isDirty(media: KelurahanMedia) {
        const f = fieldsFor(media)
        return f.alt !== (media.alt ?? "") || f.copyright !== (media.copyright ?? "")
    }

    function updateField(mediaId: string, media: KelurahanMedia, patch: Partial<{ alt: string; copyright: string }>) {
        setEdits((prev) => ({
            ...prev,
    ...
    async function handleFileSelected(e: React.ChangeEvent<HTMLInputElement>) {
        ...
        try {
            ...
            const res = await fetch("/api/kelurahan/media", { method: "POST", body: formData })
            const data = await res.json<{ media?: KelurahanMedia; error?: string }>().catch(() => null)
    ...
    async function handleDelete(mediaId: string) {
        ...
        setError(null)
        try {
            const res = await fetch(`/api/kelurahan/media?kelurahanId=${kelurahanId}&mediaId=${mediaId}`, { method: "DELETE" })
    ...
    async function handleSave(media: KelurahanMedia) {
        const f = fieldsFor(media)
        setSavingId(media.id)
        setError(null)
        try {
            const res = await fetch("/api/kelurahan/media", {
                method: "PATCH",
